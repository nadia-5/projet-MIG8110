...
commands = [
    ...
    ,
        <<-EOT
    SELECT audit.audit_table('public.order_item');
    EOT
  ]
    depends_on = [ postgresql_script.order, postgresql_script.product, postgresql_script.seller, postgresql_function.audit_table ]
}
